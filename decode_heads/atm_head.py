...
class ATMHead(BaseDecodeHead):
    ...
    def semantic_inference(self, mask_cls, mask_pred):
        ...
        # ignore
        bg = torch.ones(semseg.shape[0], 1, semseg.shape[2], semseg.shape[3]).to(semseg) * 0.5
        semseg = torch.cat([semseg, bg], axis = 1)
        return semseg
    ...
    def loss(self, inputs, batch_data_samples,
        ...
        # print("loss: ", seg_logit.keys(), seg_label.shape, self.ignore_index)
        # dict_keys(['pred_logits', 'pred_masks', 'pred', 'aux_outputs'])
    ...
    def predict(self, inputs, batch_img_metas,test_cfg) -> Tensor:
