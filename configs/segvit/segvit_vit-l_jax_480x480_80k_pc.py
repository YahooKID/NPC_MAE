...
data_preprocessor = dict(  # 数据预处理的配置项，通常包括图像的归一化和增强
    ...
    mean=[271.071128110474, 191.91817864260625, 340.61555045919664],  # 用于归一化输入图像的平均值
    std=[621.4404906452228, 376.4773954680275, 513.4203041869922],  # 用于归一化输入图像的标准差
...
model = dict(
    ...
    test_cfg=dict(mode='slide', crop_size=(img_size, img_size), stride=(149, 149)),
)
...
optimizer = dict(type='AdamW', lr=0.002, betas=(0.9, 0.999), weight_decay=0.03,
                 )
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=optimizer,
...
train_cfg = dict(type='IterBasedTrainLoop', max_iters=60900, val_interval=100)
...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', by_epoch=False, interval=100, max_keep_ckpts=3),
